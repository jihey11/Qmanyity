...
export const COSTUME_ITEMS = Object.freeze([
  ...
  Object.freeze({
    id: "teddy_bear",
    name: "곰돌이",
    description: "찹쌀떡을 먹고싶어하는 곰도리",
    category: "머리",
    layer: "head",
    asset: "/assets/character/head/teddy_bear.png",
    price: 200,
    active: true
  })
]);
...
export function getCostumePrice(item) {
  const rawPrice = item?.price;

  // 코스튬에 price가 있으면 해당 가격 사용
  if (
    rawPrice !== null &&
    rawPrice !== undefined &&
    rawPrice !== ""
  ...
  // price가 없으면 기본 가격 사용
  const defaultPrice =
    Number(COSTUME_SHOP_CONFIG.defaultPrice);

  if (
    Number.isInteger(defaultPrice) &&
    defaultPrice >= 0
  ) {
    return defaultPrice;
  }
